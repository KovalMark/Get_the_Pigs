...
// MARK: OriginalPig
class BodyPigController: UIViewController, PigAnimatable {
    ...
    private let pigButton: UIButton = {
        ...
        let image = UIImage(named: "miniPigDeletePic1")
    ...
    @objc func pressPigButton() {
        guard let sound = pigPlay.arraySoftPigSound.randomElement() else {
            pigPlay.soundsPlayer(typeArray: pigPlay.arraySoftPigSound[0])
            return
        }
